...
numComb = nchoosek(numAlleles, 2) + numAlleles;
genotypeFactor.card = [numComb, numComb, numComb];
...
for k = 1 : length(genotypeFactor.val)
	childAlleles = genotypesToAlleles(assignments(k, 1), :);
	parent1Alleles = genotypesToAlleles(assignments(k, 2), :);
	parent2Alleles = genotypesToAlleles(assignments(k, 3), :);
	count = 0;
	totalPossibleOutcomes = 4;	% (2 choose 1) * (2 choose 1)
	if (childAlleles(1) == childAlleles(2))
		count += histc(parent1Alleles, childAlleles(1)) * histc(parent2Alleles, childAlleles(2));
	else
		count += histc(parent1Alleles, childAlleles(1)) * histc(parent2Alleles, childAlleles(2));
		count += histc(parent1Alleles, childAlleles(2)) * histc(parent2Alleles, childAlleles(1));
	end	
	genotypeFactor.val(k) = count / totalPossibleOutcomes;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%