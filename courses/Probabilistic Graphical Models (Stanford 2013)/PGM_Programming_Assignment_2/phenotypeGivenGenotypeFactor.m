...
% Find phenotypes having the physical trait
ptraits = find(assignments(:, 1) == 1);
j = 1;
for i = 1:length(ptraits)
	phenotypeFactor.val(ptraits(i)) = alphaList(j);
	j = j + 1;
end
% Find phenotypes not having the physical trait
% ptraits = setdiff(AssignmentToIndex(assignments, phenotypeFactor.card), ptraits);
ptraits = find(assignments(:, 1) == 2);
j = 1;
for i = 1:length(ptraits)
	phenotypeFactor.val(ptraits(i)) = 1 - alphaList(j);
	j = j + 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%