...
for i = 1 : numPeople
	...
	if (parents(1) == 0 && parents(2) == 0)	% Type 2
		factorList(j) = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
	else % if (parents(1) <> 0 && parents(2) <> 0)	% Type 3
		factorList(j) = genotypeGivenParentsGenotypesFactor(numAlleles, i, parents(1), parents(2));
	end
	j = j + 1;
end
