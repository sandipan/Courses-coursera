...
% Find homozygous dominant or heterozygous genotypes and phenotypes having the physical trait
ptraits = intersect(find(assignments(:, 2) <= 2), find(assignments(:, 1) == 1));
for i = 1:length(ptraits)
	phenotypeFactor.val(ptraits(i)) = 1;
end
% Find homozygous recessive genotype and phenotype not having the physical trait
ptraits = intersect(find(assignments(:, 2) == 3), find(assignments(:, 1) == 2));
for i = 1:length(ptraits)
	phenotypeFactor.val(ptraits(i)) = 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
