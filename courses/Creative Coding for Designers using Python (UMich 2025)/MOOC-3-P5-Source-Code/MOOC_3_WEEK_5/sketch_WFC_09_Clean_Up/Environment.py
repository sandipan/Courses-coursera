...
class Environment:
    ...
    def all_cells_collapsed(self):
        # Check if all cells in the grid have been collapsed to a single tile.
        for x in range(self.cols):
            for y in range(self.rows):
                if len(self.cells[x][y]) > 1:  # If a cell has more than one possible tile configuration, return False.
                    return False
        return True  # If all cells have been collapsed, return True.
