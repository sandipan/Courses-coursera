...
class Force:
    # Initialize a Force object with a position and a vector representing the force
    ...
    # Execute the display function to visualize the force
    def run(self):
        self.display()
    ...
    # Highlight the force with a semi-transparent square
    def display_highlight(self, h_size):
        stroke(255, 50)  # Set the stroke color to white with some transparency
        noFill()  # Do not fill the rectangle
        rectMode(CENTER)  # Draw the rectangle from its center
        rect(self.pos.x, self.pos.y, h_size, h_size)  # Draw a rectangle to highlight the force
