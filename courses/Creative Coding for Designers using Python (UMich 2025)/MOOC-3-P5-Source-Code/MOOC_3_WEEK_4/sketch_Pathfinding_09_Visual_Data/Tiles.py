...
class Tile:
    ...
    def display_visual_data(self):
        # Map the 'f' value (total cost) to a visual color spectrum for display.
        mapped_f = map(self.f, 0, 1000, 0, 255)  # Map 'f' value to a range suitable for color coding.
        col = color(mapped_f, 255, 0)  # Create a color based on the mapped 'f' value.
        fill(col)  # Set the fill color for the cell based on the 'f' value.
        noStroke()  # Disable stroke for a cleaner look.
        rect(self.pos.x, self.pos.y, self.cell_size, self.cell_size)  # Draw the cell with the new fill color.
        
    def display(self):
        # Display the tile with visual distinctions based on its type.
        stroke(255, 50)  # Set the stroke color with partial transparency for visibility.

        # Define colors for visited and non-visited states (unused here but could be used for other visual cues).
        non_visit_color = color(255)  # White for non-visited or default state.
        visited_color = color(0, 0, 255)  # Blue for visited state.
        
        # Fill and draw the tile based on its current type.
        if self.types[self.current_type] == "floor":
            fill(0)  # Black fill for "floor" tiles.
            rect(self.pos.x, self.pos.y, self.cell_size, self.cell_size)
        elif self.types[self.current_type] == "wall":
            fill(255)  # White fill for "wall" tiles, indicating an obstacle.
            rect(self.pos.x, self.pos.y, self.cell_size, self.cell_size)
        elif self.types[self.current_type] == "resource":
            fill(255)  # White background for "resource" tiles.
            rect(self.pos.x, self.pos.y, self.cell_size, self.cell_size)
            ellipseMode(CORNER)  # Set mode for drawing ellipse to CORNER.
            fill(0, 150, 0)  # Green fill for the ellipse, representing a resource.
            ellipse(self.pos.x, self.pos.y, self.cell_size, self.cell_size)  # Draw the ellipse on the tile.
