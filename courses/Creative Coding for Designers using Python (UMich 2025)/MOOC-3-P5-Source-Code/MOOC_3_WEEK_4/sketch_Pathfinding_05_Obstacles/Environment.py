...
class Environment:
    ...
    def flood_fill(self):
        # Perform the flood fill algorithm if the simulation is running.
        if self.running:        
            if len(self.stack) > 0:  # Check if there are cells in the stack.
                # Remove and retrieve the first cell in the stack.
                current_cell = self.stack.pop(0)
                current_cell.visited = True  # Mark the current cell as visited.
                
                # If the current cell is the end node, conclude the search.
                if current_cell == self.end_node:
                    println("Goal Reached!")  # Indicate goal achievement.
                    self.running = False  # Stop the flood fill process.
            
                # Iterate through unvisited and non-obstacle neighbors.
                for neighbor in current_cell.get_neighbors():
                    if not neighbor.visited and not neighbor.isObstable:
                        ...
                        self.stack.append(neighbor)  # Add the neighbor to the stack for exploration.
    ...
    def paint_cell(self, x, y, type):
        ...
        if(cell is not None): # If the cell exists...
            ...
            cell.change_type(type) # Change the cell's type to the specified type.
