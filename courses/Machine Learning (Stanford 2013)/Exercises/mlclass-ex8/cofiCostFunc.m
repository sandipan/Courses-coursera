...
for i = 1:num_movies
	idx = find(R(i, :)==1); % users that rated i th movie
	Theta_temp = Theta(idx, :);
	Y_temp = Y(i, idx); % users that rated i th movie
	X_grad(i, :) = (X(i, :) * Theta_temp' - Y_temp)*Theta_temp + lambda * X(i, :);
end

for j = 1:num_users
	idx = find(R(:, j)==1); % movies that j th user has rated
	X_temp = X(idx, :);
	Y_temp = Y(idx, j); % movies that j th user has rated
