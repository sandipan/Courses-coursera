...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    int theadId = blockId * (blockDim.x * blockDim.y * blockDim.z) + (threadIdx.z * blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x) + threadIdx.x;
    //d_gray[theadId] = (uchar)(((int)d_r[theadId] + (int)d_g[theadId] + (int)d_b[theadId])/3.0);
    d_gray[theadId] = (uchar)(0.2126*d_r[theadId] + 0.7152*d_g[theadId] + 0.0722*d_b[theadId]);
}
...
__host__ float compareGrayImages(uchar *gray, uchar *test_gray, int rows, int columns)
{
    ...
    for(int r = 0; r < rows; ++r)
    {
        for(int c = 0; c < columns; ++c)
        {
            ...
            //cout << int(image0Pixel) << ' ' << int(image1Pixel) << '\n';
