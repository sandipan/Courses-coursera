 ...
 *
 */
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b)
{
    ...
    extern __shared__ int d_gray[];
    int lindex = (threadIdx.y * blockDim.x) + threadIdx.x;
    ...
    if(threadId < num_image_pixels)
    {
        ...
        if (lindex < blockDim.x * blockDim.y)
            d_gray[lindex] = 0; 
        ...
        if (lindex < blockDim.x * blockDim.y)
            d_gray[lindex] = (uchar)(0.2126*d_r[threadId] + 0.7152*d_g[threadId] + 0.0722*d_b[threadId]);
            d_r[threadId] = d_gray[lindex];

        //g = (uchar)(0.2126*rgb[0] + 0.7152*rgb[1] + 0.0722*rgb[2]);
        //d_r[threadId] = g;
        //d_g[threadId] = g;
        //d_b[threadId] = g;
    }
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, int rows, int columns, int threadsPerBlock)
{
    ...
    convert<<<blocksPerGrid, threadsPerBlock>>>(d_r, d_g, d_b);
