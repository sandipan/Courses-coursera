...
batch_size = 4
trainset = torchvision.datasets.ImageFolder(root='065_CNN_AudioClassification/train', transform=transform)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True)
testset = torchvision.datasets.ImageFolder(root='065_CNN_AudioClassification/test', transform=transform)
...
class ImageMulticlassClassificationNet(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=6, kernel_size= 3, padding=1)  # BS, 6, 100, 100
        self.pool = nn.MaxPool2d(2, 2)                                                    # BS, 6, 50, 50
        self.conv2 = nn.Conv2d(in_channels=6, out_channels= 16, kernel_size=3, padding=1) # BS, 16, 25, 25 # BS, 16, 25, 25
...
for i, data in enumerate(testloader, 0):
    ...
    with torch.no_grad():
        ...
        #print(y_test_temp, y_test_hat_temp)
