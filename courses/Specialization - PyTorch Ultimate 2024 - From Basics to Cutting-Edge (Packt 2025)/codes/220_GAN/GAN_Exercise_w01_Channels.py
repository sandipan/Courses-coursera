...
plt.tight_layout()
plt.savefig("./data/heart/heart_01.jpg", bbox_inches='tight') #../data
plt.close()
...
for epoch in range(NUM_EPOCHS):
    for batch_idx, (real_samples, _) in enumerate(train_loader):
        ...
        real_samples = real_samples.view(-1, image_dim).to(device)
        real_samples_labels = torch.ones((BATCH_SIZE, 1)).to(device)
        ## Creating random samples
        latent_space_samples = torch.randn((BATCH_SIZE, z_dim)).to(device)
        ...
        if epoch % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                plt.imshow(generated_samples_processed.squeeze().cpu().numpy(), cmap='gray') ##'viridis', 'bone'
                plt.grid(False)
                #plt.show()
                plt.savefig(f"./saved_images/saved_fig_GAN_01Ch_{epoch}_{NUM_EPOCHS}.png")
                plt.close()
                #plt.imshow(real_samples.squeeze().reshape(28,28).cpu().numpy())
                #plt.show()