...
#%% Data Import 
twitter_file = 'data/Tweets.csv' #360_NLP/
df = pd.read_csv(twitter_file).dropna().sample(1000, random_state=123).reset_index(drop=True)
df.head()
# %% Sentiment Analysis
#----------------------
# sentiment_pipeline = pipeline("sentiment-analysis")
sentiment_pipeline = pipeline(model="nlptown/bert-base-multilingual-uncased-sentiment", device='cuda')
data = df.loc[2, 'text']
...
# %%
print(closest_description('this is amazing'))
# %%
