...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] adj2Matrix = new int[n][n];
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j){
				adj2Matrix[i][j] = 0;
				for (int k = 0; k < n; ++k) {
					adj2Matrix[i][j] += adjMatrix[i][k]*adjMatrix[k][j];
				}
			}
		}
		Set<Integer> vertices2 = new HashSet<Integer>();
		for (int i = 0; i < n; ++i) {
			System.out.print(adj2Matrix[v][i] + ",");
			if (adj2Matrix[v][i] == 2) {
				vertices2.add(i);
			}
		}			
		*/
		Set<Integer> vertices2 = new HashSet<Integer>();
		for (Integer vertex:getNeighbors(v)) {
			 for (Integer vertex2:getNeighbors(vertex)) {
				 vertices2.add(vertex2);
			 }
		}
		List<Integer> vertices2l = new ArrayList<Integer>(); 
		vertices2l.addAll(vertices2);
		return vertices2l;	
	}
