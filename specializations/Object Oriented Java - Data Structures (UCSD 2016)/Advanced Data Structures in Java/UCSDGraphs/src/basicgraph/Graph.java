...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		int [] degreeCount = new int[2*numVertices];
		for (int d = 0; d < 2*numVertices; ++d) {
			degreeCount[d] = 0;
		}
		for (int v = 0; v < numVertices; ++v) {
			int d = getNeighbors(v).size() + getInNeighbors(v).size(); 
			++degreeCount[d];
		}
		List<Integer> degSequence = new ArrayList<Integer>();
		for (int d = 2*numVertices-1; d >= 0; --d) {
			int count = degreeCount[d];
			while (count > 0) {
				degSequence.add(d);
				System.out.print(d + ",");
				--count;
